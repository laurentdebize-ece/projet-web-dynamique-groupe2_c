...
if (!empty($nom_competence)) {
    $sql = "UPDATE competences_transversales SET nom_competences = :nom_competence WHERE id_competence = :id_competence";
...
if (!empty($id_matiere)) {
    $sql = "UPDATE compet_trans_matiere SET id_matiere = :id_matiere WHERE id_competence = :id_competence";
    $stmt = $bdd->prepare($sql);
    $stmt->execute(array(':id_matiere' => $id_matiere, ':id_competence' => $id_competence));
}

if ($stmt->rowCount() > 0) {
    $message = "Succès : La compétence transversale $nom_competence a été modifiée avec succès";
